...
class DashboardController extends Controller
{
    function dashboard(Request $request){
        ...
        $siswa = count(TransaksiHead::where('tanggal',DB::raw('curdate()'))->get());
        ...
        $pengeluaran = TransaksiHead:: select( DB::raw('sum(keluar) as pengeluaran'))->where(DB::raw('Year(tanggal)'),DB::raw('Year(now())'))->groupBy(DB::raw('MONTH(tanggal)'))->orderBy(DB::raw('MONTH(tanggal)'),'asc')->get();
        $pemasukan = TransaksiHead:: select( DB::raw('sum(masuk) as pemasukan'))->where(DB::raw('Year(tanggal)'),DB::raw('Year(now())'))->groupBy(DB::raw('MONTH(tanggal)'))->orderBy(DB::raw('MONTH(tanggal)'),'asc')->get();
        $pengeluaranTahun = TransaksiHead:: select( DB::raw('sum(keluar) as pengeluaran'))->where(DB::raw('Year(tanggal)'),DB::raw('Year(now())'))->first();
        $pemasukanTahun = TransaksiHead:: select( DB::raw('sum(masuk) as pemasukan'))->where(DB::raw('Year(tanggal)'),DB::raw('Year(now())'))->first();
