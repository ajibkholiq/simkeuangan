...
class PengeluaranController extends Controller
{
    function index(){
        ...
        $unit = MasterUnit::all();
        $akun = Akun::join('sub2_akuns','sub2_akuns.id','sub2_akun_id')->join('master_akun_head_sub','master_akun_head_sub.id','sub_akun_id')
                    ->join('akun_head','akun_head.id','akun_head_id')->where('akun_head.akun_head','beban')->select('akuns.kode','akuns.Nama')->get(); 
        ...
        return view('page.proses.pengeluaran',compact(['menu','akun','unit','via']));
        // return $akun;
    }
     function store(Request $request){
        ...
        if ($request->uuid){
            ...
            'keluar' => $data->keluar + $request->jumlah,
        ...
        return view('page.proses.pengeluaranDetail',compact(['menu','akun','data','via','detail']));

        }
         $uuid = uniqid();
        TransaksiHead::create([
            'uuid' => $uuid,
            'kode_trans' => 'PENGELUARAN',
     ...
     function show($id){
        $a = TransaksiHead::where('transaksi_heads.uuid',$id)->join('akuns','kode_akun','akuns.kode')->select('tanggal','transaksi_heads.nama','keluar','transaksi_heads.id','akuns.Nama as akun')->first();
